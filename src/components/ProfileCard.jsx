...
const ProfileCardComponent = ({
  ...
  const handlePointerEnter = useCallback(
    event => {
      ...
      shell.style.setProperty('--card-opacity', '1');
  ...
  const handlePointerLeave = useCallback(() => {
    const shell = shellRef.current;
    if (!shell || !tiltEngine) return;
    shell.style.setProperty('--card-opacity', '0');
    tiltEngine.toCenter();
  ...
  const cardStyle = useMemo(
    () => ({
     ...
     '--sunpillar-1': 'hsl(210, 35%, 72%)',
    '--sunpillar-2': 'hsl(220, 30%, 70%)',
    '--sunpillar-3': 'hsl(200, 28%, 74%)',
    '--sunpillar-4': 'hsl(240, 22%, 76%)',
    '--sunpillar-5': 'hsl(215, 26%, 72%)',
    '--sunpillar-6': 'hsl(230, 24%, 74%)',
  ...
  const shineStyle = {
   ...
   filter: `
brightness(calc(0.55 + (var(--card-opacity) * 0.18)))
contrast(calc(1 + (var(--card-opacity) * 0.12)))
saturate(calc(0 + (var(--card-opacity) * 0.55)))
opacity(calc(0.06 + (var(--card-opacity) * 0.22)))
`,
